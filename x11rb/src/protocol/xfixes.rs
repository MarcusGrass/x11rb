...
impl core::fmt::Debug for SelectionEvent {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::SET_SELECTION_OWNER.0.into(),
                "SET_SELECTION_OWNER",
                "SetSelectionOwner",
            ),
            (
                Self::SELECTION_WINDOW_DESTROY.0.into(),
                "SELECTION_WINDOW_DESTROY",
                "SelectionWindowDestroy",
            ),
            (
                Self::SELECTION_CLIENT_CLOSE.0.into(),
...
crate::bitmask_binop!(SelectionEventMask, u8);
...
crate::bitmask_binop!(CursorNotifyMask, u8);
...
impl GetCursorImageRequest {
    ...
    #[must_use]
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let mut request0 = [major_opcode, GET_CURSOR_IMAGE_REQUEST, 0, 0];
...
impl TryParse for GetCursorImageReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (cursor_serial, remaining) = u32::try_parse(remaining)?;
        let remaining = remaining.get(8..).ok_or(ParseError::InsufficientData)?;
        let (cursor_image, remaining) = crate::x11_utils::parse_list::<u32>(
            remaining,
            u32::from(width)
        ...
        let result = GetCursorImageReply {
            sequence,
            length,
            x,
            y,
...
impl GetCursorImageAndNameRequest {
    ...
    #[must_use]
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let mut request0 = [major_opcode, GET_CURSOR_IMAGE_AND_NAME_REQUEST, 0, 0];
        request0[2..4].copy_from_slice(&(1u16).to_ne_bytes());
        request0
    }
}
...
impl TryParse for GetCursorImageAndNameReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetCursorImageAndNameReply {
            sequence,
            length,
            x,
            y,
...
crate::bitmask_binop!(BarrierDirections, u8);
...
impl<'input> CreatePointerBarrierRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let num_devices =
            u16::try_from(self.devices.len()).expect("`devices` has too many elements");
        ...
        let referenced: &[u8] = devices_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(devices_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
