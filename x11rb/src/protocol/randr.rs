...
crate::bitmask_binop!(Rotation, u8);
...
impl TryParse for RefreshRates {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (rates, remaining) =
            crate::x11_utils::parse_list::<u16>(remaining, n_rates.try_to_usize()?)?;
...
impl RefreshRates {
    ...
    pub fn n_rates(&self) -> u16 {
        self.rates.len().try_into().unwrap()
    }
}
...
impl core::fmt::Debug for SetConfig {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            ...
            (
                Self::INVALID_CONFIG_TIME.0.into(),
                "INVALID_CONFIG_TIME",
                "InvalidConfigTime",
            ),
...
impl TryParse for SetScreenConfigReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = SetScreenConfigReply {
            status,
            sequence,
            length,
            new_timestamp,
...
impl core::fmt::Debug for NotifyMask {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::SCREEN_CHANGE.0.into(),
                "SCREEN_CHANGE",
                "ScreenChange",
            ),
            (Self::CRTC_CHANGE.0.into(), "CRTC_CHANGE", "CrtcChange"),
            (
                Self::OUTPUT_CHANGE.0.into(),
                "OUTPUT_CHANGE",
                "OutputChange",
            ),
            (
                Self::OUTPUT_PROPERTY.0.into(),
                "OUTPUT_PROPERTY",
                "OutputProperty",
            ),
            (
                Self::PROVIDER_CHANGE.0.into(),
                "PROVIDER_CHANGE",
                "ProviderChange",
            ),
            (
                Self::PROVIDER_PROPERTY.0.into(),
                "PROVIDER_PROPERTY",
                "ProviderProperty",
            ),
            (
                Self::RESOURCE_CHANGE.0.into(),
                "RESOURCE_CHANGE",
                "ResourceChange",
            ),
...
crate::bitmask_binop!(NotifyMask, u8);
...
impl TryParse for GetScreenInfoReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (sizes, remaining) =
            crate::x11_utils::parse_list::<ScreenSize>(remaining, n_sizes.try_to_usize()?)?;
        let (rates, remaining) = crate::x11_utils::parse_list::<RefreshRates>(
            remaining,
            u32::from(n_info)
                .checked_sub(u32::from(n_sizes))
...
impl core::fmt::Debug for ModeFlag {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::HSYNC_POSITIVE.0.into(),
                "HSYNC_POSITIVE",
                "HsyncPositive",
            ),
            (
                Self::HSYNC_NEGATIVE.0.into(),
                "HSYNC_NEGATIVE",
                "HsyncNegative",
            ),
            (
                Self::VSYNC_POSITIVE.0.into(),
                "VSYNC_POSITIVE",
                "VsyncPositive",
            ),
            (
                Self::VSYNC_NEGATIVE.0.into(),
                "VSYNC_NEGATIVE",
                "VsyncNegative",
            ),
            ...
            (
                Self::CSYNC_POSITIVE.0.into(),
                "CSYNC_POSITIVE",
                "CsyncPositive",
            ),
            (
                Self::CSYNC_NEGATIVE.0.into(),
                "CSYNC_NEGATIVE",
                "CsyncNegative",
            ),
            (
                Self::HSKEW_PRESENT.0.into(),
                "HSKEW_PRESENT",
                "HskewPresent",
            ),
            (Self::BCAST.0.into(), "BCAST", "Bcast"),
            (
                Self::PIXEL_MULTIPLEX.0.into(),
                "PIXEL_MULTIPLEX",
                "PixelMultiplex",
            ),
...
crate::bitmask_binop!(ModeFlag, u16);
...
impl TryParse for GetScreenResourcesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (crtcs, remaining) =
            crate::x11_utils::parse_list::<Crtc>(remaining, num_crtcs.try_to_usize()?)?;
        let (outputs, remaining) =
            crate::x11_utils::parse_list::<Output>(remaining, num_outputs.try_to_usize()?)?;
        let (modes, remaining) =
            crate::x11_utils::parse_list::<ModeInfo>(remaining, num_modes.try_to_usize()?)?;
        let (names, remaining) =
            crate::x11_utils::parse_u8_list(remaining, names_len.try_to_usize()?)?;
        let names = names.to_vec();
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetScreenResourcesReply {
            sequence,
            length,
            timestamp,
            config_timestamp,
...
impl GetScreenResourcesReply {
    ...
    pub fn num_crtcs(&self) -> u16 {
        self.crtcs.len().try_into().unwrap()
    }
    ...
    pub fn num_modes(&self) -> u16 {
        self.modes.len().try_into().unwrap()
    }
...
impl TryParse for GetOutputInfoReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (crtcs, remaining) =
            crate::x11_utils::parse_list::<Crtc>(remaining, num_crtcs.try_to_usize()?)?;
        let (modes, remaining) =
            crate::x11_utils::parse_list::<Mode>(remaining, num_modes.try_to_usize()?)?;
        let (clones, remaining) =
            crate::x11_utils::parse_list::<Output>(remaining, num_clones.try_to_usize()?)?;
        let (name, remaining) =
...
impl GetOutputInfoReply {
    ...
    pub fn num_clones(&self) -> u16 {
        self.clones.len().try_into().unwrap()
    }
...
impl TryParse for ListOutputPropertiesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = ListOutputPropertiesReply {
            sequence,
            length,
            atoms,
        };
...
impl TryParse for QueryOutputPropertyReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (valid_values, remaining) =
            crate::x11_utils::parse_list::<i32>(remaining, length.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = QueryOutputPropertyReply {
            sequence,
            pending,
            range,
            immutable,
...
impl QueryOutputPropertyReply {
    ...
    pub fn length(&self) -> u32 {
        self.valid_values.len().try_into().unwrap()
    }
}
...
impl TryParse for GetOutputPropertyReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetOutputPropertyReply {
            format,
            sequence,
            length,
            type_,
...
impl TryParse for GetCrtcInfoReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (outputs, remaining) =
            crate::x11_utils::parse_list::<Output>(remaining, num_outputs.try_to_usize()?)?;
        let (possible, remaining) = crate::x11_utils::parse_list::<Output>(
            remaining,
            num_possible_outputs.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let status = status.into();
        let result = GetCrtcInfoReply {
            status,
            sequence,
            length,
            timestamp,
...
impl GetCrtcInfoReply {
    ...
    pub fn num_possible_outputs(&self) -> u16 {
        self.possible.len().try_into().unwrap()
    }
}
...
impl<'input> SetCrtcConfigRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = outputs_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(outputs_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for GetCrtcGammaSizeReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetCrtcGammaSizeReply {
            sequence,
            length,
            size,
        };
...
impl TryParse for GetCrtcGammaReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (red, remaining) =
            crate::x11_utils::parse_list::<u16>(remaining, size.try_to_usize()?)?;
        let (green, remaining) =
            crate::x11_utils::parse_list::<u16>(remaining, size.try_to_usize()?)?;
        let (blue, remaining) =
            crate::x11_utils::parse_list::<u16>(remaining, size.try_to_usize()?)?;
...
impl<'input> SetCrtcGammaRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = red_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        debug_assert_eq!(
            self.green.len(),
            usize::try_from(size).unwrap(),
            "`green` has an incorrect length"
        );
        let green_bytes = self.green.serialize();
        let referenced: &[u8] = green_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        debug_assert_eq!(
            self.blue.len(),
            usize::try_from(size).unwrap(),
            "`blue` has an incorrect length"
        );
        let blue_bytes = self.blue.serialize();
        let referenced: &[u8] = blue_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(red_bytes.as_ref());
        request0.extend_from_slice(green_bytes.as_ref());
        request0.extend_from_slice(blue_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for GetScreenResourcesCurrentReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetScreenResourcesCurrentReply {
            sequence,
            length,
            timestamp,
            config_timestamp,
...
crate::bitmask_binop!(Transform, u8);
...
impl<'input> SetCrtcTransformRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let filter_len =
            u16::try_from(self.filter_name.len()).expect("`filter_name` has too many elements");
        ...
        let length_so_far = length_so_far + request0.len();
        let referenced: &[u8] = self.filter_name.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let referenced: &[u8] = padding0.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let filter_params_bytes = self.filter_params.serialize();
        let referenced: &[u8] = filter_params_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        ...
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(self.filter_name.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0.extend_from_slice(filter_params_bytes.as_ref());
...
impl TryParse for GetCrtcTransformReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (pending_filter_name, remaining) =
            crate::x11_utils::parse_u8_list(remaining, pending_len.try_to_usize()?)?;
        ...
        let remaining = remaining
            .get(misalignment..)
            .ok_or(ParseError::InsufficientData)?;
        let (pending_params, remaining) = crate::x11_utils::parse_list::<render::Fixed>(
            remaining,
            pending_nparams.try_to_usize()?,
        )?;
        let (current_filter_name, remaining) =
            crate::x11_utils::parse_u8_list(remaining, current_len.try_to_usize()?)?;
        ...
        let remaining = remaining
            .get(misalignment..)
            .ok_or(ParseError::InsufficientData)?;
        let (current_params, remaining) = crate::x11_utils::parse_list::<render::Fixed>(
            remaining,
            current_nparams.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetCrtcTransformReply {
            sequence,
            length,
            pending_transform,
            has_transforms,
...
impl GetCrtcTransformReply {
    ...
    pub fn pending_len(&self) -> u16 {
        self.pending_filter_name.len().try_into().unwrap()
    }
    ...
    pub fn pending_nparams(&self) -> u16 {
        self.pending_params.len().try_into().unwrap()
    }
    ...
    pub fn current_len(&self) -> u16 {
        self.current_filter_name.len().try_into().unwrap()
    }
    ...
    pub fn current_nparams(&self) -> u16 {
        self.current_params.len().try_into().unwrap()
    }
}
...
impl TryParse for GetProvidersReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (providers, remaining) =
            crate::x11_utils::parse_list::<Provider>(remaining, num_providers.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetProvidersReply {
            sequence,
            length,
            timestamp,
            providers,
...
impl GetProvidersReply {
    ...
    pub fn num_providers(&self) -> u16 {
        self.providers.len().try_into().unwrap()
    }
}
...
impl core::fmt::Debug for ProviderCapability {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::SOURCE_OUTPUT.0.into(),
                "SOURCE_OUTPUT",
                "SourceOutput",
            ),
            (Self::SINK_OUTPUT.0.into(), "SINK_OUTPUT", "SinkOutput"),
            (
                Self::SOURCE_OFFLOAD.0.into(),
                "SOURCE_OFFLOAD",
                "SourceOffload",
            ),
...
crate::bitmask_binop!(ProviderCapability, u8);
...
impl TryParse for GetProviderInfoReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (outputs, remaining) =
            crate::x11_utils::parse_list::<Output>(remaining, num_outputs.try_to_usize()?)?;
        let (associated_providers, remaining) = crate::x11_utils::parse_list::<Provider>(
            remaining,
            num_associated_providers.try_to_usize()?,
        )?;
        let (associated_capability, remaining) = crate::x11_utils::parse_list::<u32>(
            remaining,
            num_associated_providers.try_to_usize()?,
        )?;
        let (name, remaining) =
        ...
        let result = GetProviderInfoReply {
            status,
            sequence,
            length,
            timestamp,
...
impl GetProviderInfoReply {
    ...
    pub fn num_associated_providers(&self) -> u16 {
        self.associated_providers.len().try_into().unwrap()
    }
    /// Get the value of the `name_len` field.
    ///
    /// The `name_len` field is used as the length field of the `name` field.
...
impl TryParse for ListProviderPropertiesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = ListProviderPropertiesReply {
            sequence,
            length,
            atoms,
        };
...
impl TryParse for QueryProviderPropertyReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = QueryProviderPropertyReply {
            sequence,
            pending,
            range,
            immutable,
...
impl<'input> ChangeProviderPropertyRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        debug_assert_eq!(
            ...
            usize::try_from(
                self.num_items
                    .checked_mul(u32::from(self.format).checked_div(8u32).unwrap())
                    .unwrap()
            )
            .unwrap(),
            "`data` has an incorrect length"
...
impl TryParse for GetProviderPropertyReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetProviderPropertyReply {
            format,
            sequence,
            length,
            type_,
...
impl TryParse for ScreenChangeNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (mheight, remaining) = u16::try_parse(remaining)?;
        let subpixel_order = subpixel_order.into();
        let result = ScreenChangeNotifyEvent {
            response_type,
            rotation,
            sequence,
            timestamp,
...
impl TryParse for MonitorInfo {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (outputs, remaining) =
            crate::x11_utils::parse_list::<Output>(remaining, n_output.try_to_usize()?)?;
...
impl TryParse for GetMonitorsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(12..).ok_or(ParseError::InsufficientData)?;
        let (monitors, remaining) =
            crate::x11_utils::parse_list::<MonitorInfo>(remaining, n_monitors.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetMonitorsReply {
            sequence,
            length,
            timestamp,
            n_outputs,
...
impl GetMonitorsReply {
    ...
    pub fn n_monitors(&self) -> u32 {
        self.monitors.len().try_into().unwrap()
    }
}
...
impl SetMonitorRequest {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = monitorinfo_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(monitorinfo_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
