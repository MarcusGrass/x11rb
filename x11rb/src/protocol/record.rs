...
impl core::fmt::Debug for HType {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::FROM_SERVER_TIME.0.into(),
                "FROM_SERVER_TIME",
                "FromServerTime",
            ),
            (
                Self::FROM_CLIENT_TIME.0.into(),
                "FROM_CLIENT_TIME",
                "FromClientTime",
            ),
            (
                Self::FROM_CLIENT_SEQUENCE.0.into(),
...
impl core::fmt::Debug for CS {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::CURRENT_CLIENTS.0.into(),
                "CURRENT_CLIENTS",
                "CurrentClients",
            ),
            (
                Self::FUTURE_CLIENTS.0.into(),
                "FUTURE_CLIENTS",
                "FutureClients",
            ),
...
impl TryParse for ClientInfo {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (ranges, remaining) =
            crate::x11_utils::parse_list::<Range>(remaining, num_ranges.try_to_usize()?)?;
...
impl<'input> CreateContextRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let num_client_specs =
            u32::try_from(self.client_specs.len()).expect("`client_specs` has too many elements");
        ...
        request0.extend_from_slice(client_specs_bytes.as_ref());
        request0.extend_from_slice(ranges_bytes.as_ref());
...
impl TryParse for GetContextReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (intercepted_clients, remaining) = crate::x11_utils::parse_list::<ClientInfo>(
            remaining,
            num_intercepted_clients.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
...
impl GetContextReply {
    ...
    pub fn num_intercepted_clients(&self) -> u32 {
        self.intercepted_clients.len().try_into().unwrap()
    }
}
