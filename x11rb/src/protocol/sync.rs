...
impl ListSystemCountersRequest {
    ...
    #[must_use]
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let mut request0 = [major_opcode, LIST_SYSTEM_COUNTERS_REQUEST, 0, 0];
...
impl TryParse for ListSystemCountersReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (counters, remaining) =
            crate::x11_utils::parse_list::<Systemcounter>(remaining, counters_len.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = ListSystemCountersReply {
            sequence,
            length,
            counters,
        };
...
impl ListSystemCountersReply {
    ...
    pub fn counters_len(&self) -> u32 {
        self.counters.len().try_into().unwrap()
    }
}
...
impl<'input> AwaitRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let length_so_far = 0;
        let mut request0 = vec![major_opcode, AWAIT_REQUEST, 0, 0];
        let length_so_far = length_so_far + request0.len();
        let wait_list_bytes = self.wait_list.serialize();
        let referenced: &[u8] = wait_list_bytes.as_ref();
        ...
        request0.extend_from_slice(wait_list_bytes.as_ref());
...
impl CreateAlarmAux {
    ...
    #[must_use]
    pub fn counter<I>(mut self, value: I) -> Self
    where
    ...
    #[must_use]
    pub fn value_type<I>(mut self, value: I) -> Self
    where
    ...
    #[must_use]
    pub fn value<I>(mut self, value: I) -> Self
    where
        I: Into<Option<Int64>>,
    {
    ...
    #[must_use]
    pub fn test_type<I>(mut self, value: I) -> Self
    where
    ...
    #[must_use]
    pub fn delta<I>(mut self, value: I) -> Self
    where
        I: Into<Option<Int64>>,
    {
    ...
    #[must_use]
    pub fn events<I>(mut self, value: I) -> Self
    where
        I: Into<Option<u32>>,
    {
...
impl<'input> AwaitFenceRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let length_so_far = 0;
        let mut request0 = vec![major_opcode, AWAIT_FENCE_REQUEST, 0, 0];
        let length_so_far = length_so_far + request0.len();
        let fence_list_bytes = self.fence_list.serialize();
        let referenced: &[u8] = fence_list_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(fence_list_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
