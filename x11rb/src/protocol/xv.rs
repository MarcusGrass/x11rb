...
crate::bitmask_binop!(AttributeFlag, u8);
...
impl TryParse for AdaptorInfo {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (name, remaining) =
            crate::x11_utils::parse_u8_list(remaining, name_size.try_to_usize()?)?;
        ...
        let (formats, remaining) =
            crate::x11_utils::parse_list::<Format>(remaining, num_formats.try_to_usize()?)?;
        let result = AdaptorInfo {
            base_id,
            num_ports,
            type_,
            name,
...
impl Serialize for AdaptorInfo {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        ...
        let num_formats =
            u16::try_from(self.formats.len()).expect("`formats` has too many elements");
...
impl TryParse for Image {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (pitches, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_planes.try_to_usize()?)?;
        let (offsets, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_planes.try_to_usize()?)?;
        let (data, remaining) =
            crate::x11_utils::parse_u8_list(remaining, data_size.try_to_usize()?)?;
...
impl Serialize for Image {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        ...
        let num_planes =
            u32::try_from(self.pitches.len()).expect("`pitches` has too many elements");
        num_planes.serialize_into(bytes);
        self.pitches.serialize_into(bytes);
        debug_assert_eq!(
            self.offsets.len(),
            usize::try_from(num_planes).unwrap(),
            "`offsets` has an incorrect length"
        );
...
impl Image {
    ...
    pub fn num_planes(&self) -> u32 {
        self.pitches.len().try_into().unwrap()
    }
}
...
impl QueryExtensionRequest {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let mut request0 = [major_opcode, QUERY_EXTENSION_REQUEST, 0, 0];
...
impl TryParse for QueryAdaptorsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (info, remaining) =
            crate::x11_utils::parse_list::<AdaptorInfo>(remaining, num_adaptors.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
...
impl TryParse for QueryEncodingsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (info, remaining) =
            crate::x11_utils::parse_list::<EncodingInfo>(remaining, num_encodings.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = QueryEncodingsReply {
            sequence,
            length,
            info,
        };
...
impl TryParse for GetPortAttributeReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetPortAttributeReply {
            sequence,
            length,
            value,
        };
...
impl TryParse for QueryPortAttributesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (attributes, remaining) = crate::x11_utils::parse_list::<AttributeInfo>(
            remaining,
            num_attributes.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = QueryPortAttributesReply {
            sequence,
            length,
            text_size,
            attributes,
...
impl TryParse for ListImageFormatsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (format, remaining) = crate::x11_utils::parse_list::<ImageFormatInfo>(
            remaining,
            num_formats.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = ListImageFormatsReply {
            sequence,
            length,
            format,
        };
...
impl ListImageFormatsReply {
    ...
    pub fn num_formats(&self) -> u32 {
        self.format.len().try_into().unwrap()
    }
}
...
impl TryParse for QueryImageAttributesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = QueryImageAttributesReply {
            sequence,
            length,
            data_size,
            width,
...
impl ShmPutImageRequest {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        request0[2..4].copy_from_slice(&(13u16).to_ne_bytes());
        request0
    }
}
