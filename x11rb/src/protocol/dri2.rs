...
impl core::fmt::Debug for EventType {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::EXCHANGE_COMPLETE.0.into(),
                "EXCHANGE_COMPLETE",
                "ExchangeComplete",
            ),
            (
                Self::BLIT_COMPLETE.0.into(),
                "BLIT_COMPLETE",
                "BlitComplete",
            ),
            (
                Self::FLIP_COMPLETE.0.into(),
                "FLIP_COMPLETE",
                "FlipComplete",
            ),
        ];
...
impl TryParse for ConnectReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (driver_name, remaining) =
            crate::x11_utils::parse_u8_list(remaining, driver_name_length.try_to_usize()?)?;
        ...
        let (alignment_pad, remaining) = crate::x11_utils::parse_u8_list(
            ...
            (driver_name_length
                ...
                & (!3u32))
                .checked_sub(driver_name_length)
                .ok_or(ParseError::InvalidExpression)?
                .try_to_usize()?,
        )?;
        let alignment_pad = alignment_pad.to_vec();
        let (device_name, remaining) =
            crate::x11_utils::parse_u8_list(remaining, device_name_length.try_to_usize()?)?;
        let device_name = device_name.to_vec();
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = ConnectReply {
            sequence,
            length,
            driver_name,
            alignment_pad,
...
impl ConnectReply {
    ...
    pub fn driver_name_length(&self) -> u32 {
        self.driver_name.len().try_into().unwrap()
    }
    ...
    pub fn device_name_length(&self) -> u32 {
        self.device_name.len().try_into().unwrap()
    }
}
...
impl<'input> GetBuffersRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = attachments_bytes.as_ref();
        ...
        request0.extend_from_slice(attachments_bytes.as_ref());
...
impl TryParse for GetBuffersReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (buffers, remaining) =
            crate::x11_utils::parse_list::<DRI2Buffer>(remaining, count.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetBuffersReply {
            sequence,
            length,
            width,
            height,
...
impl TryParse for GetBuffersWithFormatReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetBuffersWithFormatReply {
            sequence,
            length,
            width,
            height,
...
impl TryParse for InvalidateBuffersEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (sequence, remaining) = u16::try_parse(remaining)?;
        let (drawable, remaining) = xproto::Drawable::try_parse(remaining)?;
        let result = InvalidateBuffersEvent {
            response_type,
            sequence,
            drawable,
        };
