...
impl<'input> GetExtensionVersionRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = self.name.as_ref();
        ...
        request0.extend_from_slice(self.name.as_ref());
...
impl TryParse for GetExtensionVersionReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetExtensionVersionReply {
            xi_reply_type,
            sequence,
            length,
            server_major,
...
impl core::fmt::Debug for DeviceUse {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            ...
            (
                Self::IS_X_EXTENSION_DEVICE.0.into(),
                "IS_X_EXTENSION_DEVICE",
                "IsXExtensionDevice",
            ),
            (
                Self::IS_X_EXTENSION_KEYBOARD.0.into(),
                "IS_X_EXTENSION_KEYBOARD",
                "IsXExtensionKeyboard",
            ),
            (
                Self::IS_X_EXTENSION_POINTER.0.into(),
...
impl TryParse for ValuatorInfo {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (axes, remaining) =
            crate::x11_utils::parse_list::<AxisInfo>(remaining, axes_len.try_to_usize()?)?;
        let class_id = class_id.into();
        let mode = mode.into();
        let result = ValuatorInfo {
            class_id,
            len,
            mode,
            motion_size,
...
impl TryParse for InputInfoInfoValuator {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (axes, remaining) =
            crate::x11_utils::parse_list::<AxisInfo>(remaining, axes_len.try_to_usize()?)?;
        let mode = mode.into();
        let result = InputInfoInfoValuator {
            mode,
            motion_size,
            axes,
        };
    ...
    fn try_parse(value: &[u8], class_id: u8) -> Result<(Self, &[u8]), ParseError> {
        ...
        if switch_expr == u8::from(InputClass::KEY) {
            ...
            assert!(
                ...
                "The XML should prevent more than one 'if' from matching"
            );
...
impl InputInfoInfo {
    #[allow(dead_code)]
    fn serialize(&self, class_id: u8) -> impl AsRef<[u8]> {
        let mut result = Vec::new();
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, class_id: u8) {
        debug_assert_eq!(
            self.switch_expr(),
            class_id,
            "switch `info` has an inconsistent discriminant"
        );
        match self {
...
impl ListInputDevicesRequest {
    ...
    #[must_use]
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let mut request0 = [major_opcode, LIST_INPUT_DEVICES_REQUEST, 0, 0];
...
impl TryParse for ListInputDevicesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (devices, remaining) =
            crate::x11_utils::parse_list::<DeviceInfo>(remaining, devices_len.try_to_usize()?)?;
        let (infos, remaining) = crate::x11_utils::parse_list::<InputInfo>(
            remaining,
            devices
                .iter()
                .try_fold(0u32, |acc, x| {
                    acc.checked_add(u32::from(x.num_class_info))
                        .ok_or(ParseError::InvalidExpression)
                })?
                .try_to_usize()?,
        )?;
        let (names, remaining) =
            crate::x11_utils::parse_list::<xproto::Str>(remaining, devices_len.try_to_usize()?)?;
        ...
        let result = ListInputDevicesReply {
            xi_reply_type,
            sequence,
            length,
            devices,
...
impl ListInputDevicesReply {
    ...
    pub fn devices_len(&self) -> u8 {
        self.devices.len().try_into().unwrap()
    }
}
...
impl Serialize for InputClassInfo {
    ...
    fn serialize(&self) -> [u8; 2] {
        ...
        [class_id_bytes[0], event_type_base_bytes[0]]
    }
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        bytes.reserve(2);
        u8::from(self.class_id).serialize_into(bytes);
...
impl TryParse for OpenDeviceReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(23..).ok_or(ParseError::InsufficientData)?;
        let (class_info, remaining) =
            crate::x11_utils::parse_list::<InputClassInfo>(remaining, num_classes.try_to_usize()?)?;
        // Align offset to multiple of 4
        let offset = remaining.as_ptr() as usize - value.as_ptr() as usize;
        let misalignment = (4 - (offset % 4)) % 4;
        let remaining = remaining
...
impl OpenDeviceReply {
    ...
    pub fn num_classes(&self) -> u8 {
        self.class_info.len().try_into().unwrap()
    }
}
...
impl<'input> SelectExtensionEventRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let num_classes =
            u16::try_from(self.classes.len()).expect("`classes` has too many elements");
        ...
        let referenced: &[u8] = classes_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(classes_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for GetSelectedExtensionEventsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (this_classes, remaining) = crate::x11_utils::parse_list::<EventClass>(
            remaining,
            num_this_classes.try_to_usize()?,
        )?;
        let (all_classes, remaining) =
            crate::x11_utils::parse_list::<EventClass>(remaining, num_all_classes.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
...
impl GetSelectedExtensionEventsReply {
    ...
    pub fn num_this_classes(&self) -> u16 {
        self.this_classes.len().try_into().unwrap()
    }
    ...
    pub fn num_all_classes(&self) -> u16 {
        self.all_classes.len().try_into().unwrap()
    }
}
...
impl TryParse for GetDeviceDontPropagateListReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (classes, remaining) =
            crate::x11_utils::parse_list::<EventClass>(remaining, num_classes.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
...
impl GetDeviceDontPropagateListReply {
    ...
    pub fn num_classes(&self) -> u16 {
        self.classes.len().try_into().unwrap()
    }
}
...
impl DeviceTimeCoord {
    pub fn try_parse(remaining: &[u8], num_axes: u8) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (axisvalues, remaining) =
            crate::x11_utils::parse_list::<i32>(remaining, num_axes.try_to_usize()?)?;
...
impl DeviceTimeCoord {
    #[allow(dead_code)]
    fn serialize(&self, num_axes: u8) -> impl AsRef<[u8]> {
        let mut result = Vec::new();
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, num_axes: u8) {
        ...
        debug_assert_eq!(
            self.axisvalues.len(),
            usize::try_from(num_axes).unwrap(),
            "`axisvalues` has an incorrect length"
        );
...
impl TryParse for GetDeviceMotionEventsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetDeviceMotionEventsReply {
            xi_reply_type,
            sequence,
            length,
            num_axes,
...
impl TryParse for ChangeKeyboardDeviceReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let status = status.into();
        let result = ChangeKeyboardDeviceReply {
            xi_reply_type,
            sequence,
            length,
            status,
...
impl TryParse for ChangePointerDeviceReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let status = status.into();
        let result = ChangePointerDeviceReply {
            xi_reply_type,
            sequence,
            length,
            status,
...
impl core::fmt::Debug for DeviceInputMode {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::ASYNC_THIS_DEVICE.0.into(),
                "ASYNC_THIS_DEVICE",
                "AsyncThisDevice",
            ),
            (
                Self::SYNC_THIS_DEVICE.0.into(),
                "SYNC_THIS_DEVICE",
                "SyncThisDevice",
            ),
            (
                Self::REPLAY_THIS_DEVICE.0.into(),
                "REPLAY_THIS_DEVICE",
                "ReplayThisDevice",
            ),
            (
                Self::ASYNC_OTHER_DEVICES.0.into(),
                "ASYNC_OTHER_DEVICES",
                "AsyncOtherDevices",
            ),
...
impl TryParse for GetDeviceFocusReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetDeviceFocusReply {
            xi_reply_type,
            sequence,
            length,
            focus,
...
impl TryParse for StringFeedbackState {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (keysyms, remaining) =
            crate::x11_utils::parse_list::<xproto::Keysym>(remaining, num_keysyms.try_to_usize()?)?;
        let class_id = class_id.into();
        let result = StringFeedbackState {
            class_id,
            feedback_id,
            len,
            max_symbols,
...
impl Serialize for StringFeedbackState {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        ...
        let num_keysyms =
            u16::try_from(self.keysyms.len()).expect("`keysyms` has too many elements");
...
impl StringFeedbackState {
    ...
    pub fn num_keysyms(&self) -> u16 {
        self.keysyms.len().try_into().unwrap()
    }
}
...
impl TryParse for FeedbackStateDataKeyboard {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (auto_repeats, remaining) = crate::x11_utils::parse_u8_list(remaining, 32)?;
        let auto_repeats = <[u8; 32]>::try_from(auto_repeats).unwrap();
        let result = FeedbackStateDataKeyboard {
            pitch,
            duration,
            led_mask,
            led_values,
...
impl TryParse for FeedbackStateDataPointer {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (accel_denom, remaining) = u16::try_parse(remaining)?;
        let (threshold, remaining) = u16::try_parse(remaining)?;
        let result = FeedbackStateDataPointer {
            accel_num,
            accel_denom,
            threshold,
        };
...
impl TryParse for FeedbackStateDataString {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (keysyms, remaining) =
            crate::x11_utils::parse_list::<xproto::Keysym>(remaining, num_keysyms.try_to_usize()?)?;
        let result = FeedbackStateDataString {
            max_symbols,
            keysyms,
        };
        Ok((result, remaining))
...
impl TryParse for FeedbackStateDataInteger {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (min_value, remaining) = i32::try_parse(remaining)?;
        let (max_value, remaining) = i32::try_parse(remaining)?;
        let result = FeedbackStateDataInteger {
            resolution,
            min_value,
            max_value,
        };
...
impl TryParse for FeedbackStateDataBell {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (pitch, remaining) = u16::try_parse(remaining)?;
        let (duration, remaining) = u16::try_parse(remaining)?;
        let result = FeedbackStateDataBell {
            percent,
            pitch,
            duration,
        };
...
impl FeedbackStateData {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, class_id: u8) {
        debug_assert_eq!(
            self.switch_expr(),
            class_id,
            "switch `data` has an inconsistent discriminant"
        );
        match self {
            ...
            FeedbackStateData::InvalidValue(_) => {
                panic!("attempted to serialize invalid switch case")
            }
        }
    }
}
...
impl TryParse for GetFeedbackControlReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (feedbacks, remaining) = crate::x11_utils::parse_list::<FeedbackState>(
            remaining,
            num_feedbacks.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
...
impl GetFeedbackControlReply {
    ...
    pub fn num_feedbacks(&self) -> u16 {
        self.feedbacks.len().try_into().unwrap()
    }
}
...
impl TryParse for FeedbackCtlDataKeyboard {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (led_mask, remaining) = u32::try_parse(remaining)?;
        let (led_values, remaining) = u32::try_parse(remaining)?;
        let result = FeedbackCtlDataKeyboard {
            key,
            auto_repeat_mode,
            key_click_percent,
            bell_percent,
...
impl TryParse for FeedbackCtlDataBell {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (pitch, remaining) = i16::try_parse(remaining)?;
        let (duration, remaining) = i16::try_parse(remaining)?;
        let result = FeedbackCtlDataBell {
            percent,
            pitch,
            duration,
        };
...
impl FeedbackCtlData {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, class_id: u8) {
        ...
        match self {
            ...
            FeedbackCtlData::InvalidValue(_) => {
                panic!("attempted to serialize invalid switch case")
            }
        }
    }
...
crate::bitmask_binop!(ChangeFeedbackControlMask, u8);
...
impl ChangeFeedbackControlRequest {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = feedback_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(feedback_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for GetDeviceKeyMappingReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(23..).ok_or(ParseError::InsufficientData)?;
        let (keysyms, remaining) =
            crate::x11_utils::parse_list::<xproto::Keysym>(remaining, length.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetDeviceKeyMappingReply {
            xi_reply_type,
            sequence,
            keysyms_per_keycode,
            keysyms,
...
impl<'input> ChangeDeviceKeyMappingRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        debug_assert_eq!(
            self.keysyms.len(),
            usize::try_from(
                u32::from(self.keycode_count)
                    .checked_mul(u32::from(self.keysyms_per_keycode))
                    .unwrap()
            )
            .unwrap(),
            "`keysyms` has an incorrect length"
        );
        let keysyms_bytes = self.keysyms.serialize();
        let referenced: &[u8] = keysyms_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(keysyms_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for GetDeviceModifierMappingReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(23..).ok_or(ParseError::InsufficientData)?;
        let (keymaps, remaining) = crate::x11_utils::parse_u8_list(
            remaining,
            u32::from(keycodes_per_modifier)
                .checked_mul(8u32)
                .ok_or(ParseError::InvalidExpression)?
                .try_to_usize()?,
        )?;
...
impl<'input> SetDeviceModifierMappingRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        debug_assert_eq!(
            self.keymaps.len() % 8,
            0,
            "`keymaps` has an incorrect length, must be a multiple of 8"
        );
        let keycodes_per_modifier =
            u8::try_from(self.keymaps.len() / 8).expect("`keymaps` has too many elements");
        ...
        let length_so_far = length_so_far + request0.len();
        let referenced: &[u8] = self.keymaps.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(self.keymaps.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for SetDeviceModifierMappingReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let status = status.into();
        let result = SetDeviceModifierMappingReply {
            xi_reply_type,
            sequence,
            length,
            status,
...
impl TryParse for GetDeviceButtonMappingReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(23..).ok_or(ParseError::InsufficientData)?;
        let (map, remaining) =
            crate::x11_utils::parse_u8_list(remaining, map_size.try_to_usize()?)?;
        let map = map.to_vec();
        // Align offset to multiple of 4
        let offset = remaining.as_ptr() as usize - value.as_ptr() as usize;
        let misalignment = (4 - (offset % 4)) % 4;
        ...
        let result = GetDeviceButtonMappingReply {
            xi_reply_type,
            sequence,
            length,
            map,
...
impl<'input> SetDeviceButtonMappingRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let length_so_far = length_so_far + request0.len();
        let referenced: &[u8] = self.map.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(self.map.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for SetDeviceButtonMappingReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let status = status.into();
        let result = SetDeviceButtonMappingReply {
            xi_reply_type,
            sequence,
            length,
            status,
...
impl core::fmt::Debug for ValuatorStateModeMask {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::DEVICE_MODE_ABSOLUTE.0.into(),
                "DEVICE_MODE_ABSOLUTE",
                "DeviceModeAbsolute",
            ),
            (
                Self::OUT_OF_PROXIMITY.0.into(),
                "OUT_OF_PROXIMITY",
                "OutOfProximity",
            ),
        ];
...
crate::bitmask_binop!(ValuatorStateModeMask, u8);
...
impl TryParse for ValuatorState {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (mode, remaining) = u8::try_parse(remaining)?;
        let (valuators, remaining) =
            crate::x11_utils::parse_list::<i32>(remaining, num_valuators.try_to_usize()?)?;
        let class_id = class_id.into();
        let result = ValuatorState {
            class_id,
            len,
            mode,
            valuators,
...
impl Serialize for ValuatorState {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        ...
        self.len.serialize_into(bytes);
        let num_valuators =
            u8::try_from(self.valuators.len()).expect("`valuators` has too many elements");
...
impl ValuatorState {
    ...
    pub fn num_valuators(&self) -> u8 {
        self.valuators.len().try_into().unwrap()
    }
}
...
impl TryParse for InputStateDataButton {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (buttons, remaining) = crate::x11_utils::parse_u8_list(remaining, 32)?;
        let buttons = <[u8; 32]>::try_from(buttons).unwrap();
        let result = InputStateDataButton {
            num_buttons,
            buttons,
        };
        Ok((result, remaining))
...
impl TryParse for QueryDeviceStateReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(23..).ok_or(ParseError::InsufficientData)?;
        let (classes, remaining) =
            crate::x11_utils::parse_list::<InputState>(remaining, num_classes.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = QueryDeviceStateReply {
            xi_reply_type,
            sequence,
            length,
            classes,
...
impl<'input> SetDeviceValuatorsRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = valuators_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(valuators_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for SetDeviceValuatorsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let status = status.into();
        let result = SetDeviceValuatorsReply {
            xi_reply_type,
            sequence,
            length,
            status,
...
impl TryParse for DeviceResolutionState {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (resolution_values, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_valuators.try_to_usize()?)?;
        let (resolution_min, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_valuators.try_to_usize()?)?;
        let (resolution_max, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_valuators.try_to_usize()?)?;
        let control_id = control_id.into();
        let result = DeviceResolutionState {
            control_id,
            len,
            resolution_values,
            resolution_min,
...
impl Serialize for DeviceResolutionState {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        ...
        self.len.serialize_into(bytes);
        let num_valuators = u32::try_from(self.resolution_values.len())
            .expect("`resolution_values` has too many elements");
        num_valuators.serialize_into(bytes);
        self.resolution_values.serialize_into(bytes);
        debug_assert_eq!(
            self.resolution_min.len(),
            usize::try_from(num_valuators).unwrap(),
            "`resolution_min` has an incorrect length"
        );
        self.resolution_min.serialize_into(bytes);
        debug_assert_eq!(
            self.resolution_max.len(),
            usize::try_from(num_valuators).unwrap(),
            "`resolution_max` has an incorrect length"
        );
...
impl DeviceResolutionState {
    ...
    pub fn num_valuators(&self) -> u32 {
        self.resolution_values.len().try_into().unwrap()
    }
}
...
impl TryParse for DeviceAbsCalibState {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let control_id = control_id.into();
        let result = DeviceAbsCalibState {
            control_id,
            len,
            min_x,
            max_x,
...
impl TryParse for DeviceStateDataResolution {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (resolution_max, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_valuators.try_to_usize()?)?;
        let result = DeviceStateDataResolution {
            resolution_values,
            resolution_min,
            resolution_max,
        };
...
impl TryParse for DeviceStateDataAbsCalib {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (rotation, remaining) = u32::try_parse(remaining)?;
        let (button_threshold, remaining) = u32::try_parse(remaining)?;
        let result = DeviceStateDataAbsCalib {
            min_x,
            max_x,
            min_y,
            max_y,
...
impl Serialize for DeviceStateDataCore {
    ...
    fn serialize(&self) -> [u8; 4] {
        let status_bytes = self.status.serialize();
        let iscore_bytes = self.iscore.serialize();
        [status_bytes[0], iscore_bytes[0], 0, 0]
    }
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        bytes.reserve(4);
        self.status.serialize_into(bytes);
...
impl TryParse for DeviceStateDataAbsArea {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (screen, remaining) = u32::try_parse(remaining)?;
        let (following, remaining) = u32::try_parse(remaining)?;
        let result = DeviceStateDataAbsArea {
            offset_x,
            offset_y,
            width,
            height,
...
impl DeviceStateData {
    fn try_parse(value: &[u8], control_id: u16) -> Result<(Self, &[u8]), ParseError> {
        ...
        if switch_expr == u16::from(DeviceControl::RESOLUTION) {
            let (resolution, new_remaining) =
                DeviceStateDataResolution::try_parse(outer_remaining)?;
            outer_remaining = new_remaining;
            assert!(
                parse_result.is_none(),
                "The XML should prevent more than one 'if' from matching"
...
impl DeviceStateData {
    #[allow(dead_code)]
    fn serialize(&self, control_id: u16) -> impl AsRef<[u8]> {
        let mut result = Vec::new();
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, control_id: u16) {
        ...
        match self {
            ...
            DeviceStateData::InvalidValue(_) => {
                panic!("attempted to serialize invalid switch case")
            }
        }
    }
...
impl TryParse for DeviceResolutionCtl {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (resolution_values, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_valuators.try_to_usize()?)?;
        let control_id = control_id.into();
        let result = DeviceResolutionCtl {
            control_id,
            len,
            first_valuator,
            resolution_values,
...
impl TryParse for DeviceCtlDataResolution {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (resolution_values, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_valuators.try_to_usize()?)?;
        let result = DeviceCtlDataResolution {
            first_valuator,
            resolution_values,
        };
        Ok((result, remaining))
...
impl TryParse for DeviceCtlDataAbsCalib {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (rotation, remaining) = u32::try_parse(remaining)?;
        let (button_threshold, remaining) = u32::try_parse(remaining)?;
        let result = DeviceCtlDataAbsCalib {
            min_x,
            max_x,
            min_y,
            max_y,
...
impl TryParse for DeviceCtlDataAbsArea {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (screen, remaining) = i32::try_parse(remaining)?;
        let (following, remaining) = u32::try_parse(remaining)?;
        let result = DeviceCtlDataAbsArea {
            offset_x,
            offset_y,
            width,
            height,
...
impl ChangeDeviceControlRequest {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = control_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(control_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for ChangeDeviceControlReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = ChangeDeviceControlReply {
            xi_reply_type,
            sequence,
            length,
            status,
...
impl TryParse for ListDevicePropertiesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (atoms, remaining) =
            crate::x11_utils::parse_list::<xproto::Atom>(remaining, num_atoms.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = ListDevicePropertiesReply {
            xi_reply_type,
            sequence,
            length,
            atoms,
...
impl ListDevicePropertiesReply {
    ...
    pub fn num_atoms(&self) -> u16 {
        self.atoms.len().try_into().unwrap()
    }
}
...
impl ChangeDevicePropertyAux {
    fn try_parse(value: &[u8], format: u8, num_items: u32) -> Result<(Self, &[u8]), ParseError> {
        ...
        if switch_expr == u8::from(PropertyFormat::M8_BITS) {
            ...
            let value = remaining;
            let (data8, remaining) =
                crate::x11_utils::parse_u8_list(remaining, num_items.try_to_usize()?)?;
        ...
        if switch_expr == u8::from(PropertyFormat::M16_BITS) {
            let remaining = outer_remaining;
            let value = remaining;
            let (data16, remaining) =
                crate::x11_utils::parse_list::<u16>(remaining, num_items.try_to_usize()?)?;
            // Align offset to multiple of 4
            let offset = remaining.as_ptr() as usize - value.as_ptr() as usize;
            let misalignment = (4 - (offset % 4)) % 4;
            let remaining = remaining
        ...
        if switch_expr == u8::from(PropertyFormat::M32_BITS) {
            let remaining = outer_remaining;
            let (data32, remaining) =
                crate::x11_utils::parse_list::<u32>(remaining, num_items.try_to_usize()?)?;
            outer_remaining = remaining;
            assert!(
                parse_result.is_none(),
                "The XML should prevent more than one 'if' from matching"
...
impl ChangeDevicePropertyAux {
    #[allow(dead_code)]
    fn serialize(&self, format: u8, num_items: u32) -> impl AsRef<[u8]> {
        let mut result = Vec::new();
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, format: u8, num_items: u32) {
        debug_assert_eq!(
            self.switch_expr(),
            format,
            "switch `items` has an inconsistent discriminant"
        );
        match self {
            ChangeDevicePropertyAux::Data8(data8) => {
                debug_assert_eq!(
                    data8.len(),
                    usize::try_from(num_items).unwrap(),
                    "`data8` has an incorrect length"
                );
            ...
            ChangeDevicePropertyAux::Data16(data16) => {
                debug_assert_eq!(
                    data16.len(),
                    usize::try_from(num_items).unwrap(),
                    "`data16` has an incorrect length"
                );
            ...
            ChangeDevicePropertyAux::Data32(data32) => {
                debug_assert_eq!(
                    data32.len(),
                    usize::try_from(num_items).unwrap(),
                    "`data32` has an incorrect length"
                );
                data32.serialize_into(bytes);
            }
            ChangeDevicePropertyAux::InvalidValue(_) => {
                panic!("attempted to serialize invalid switch case")
            }
        }
    }
...
impl<'input> ChangeDevicePropertyRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = items_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(items_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for GetDevicePropertyReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetDevicePropertyReply {
            xi_reply_type,
            sequence,
            length,
            type_,
...
impl TryParse for XIQueryPointerReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (buttons, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, buttons_len.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = XIQueryPointerReply {
            sequence,
            length,
            root,
            child,
...
impl XIQueryPointerReply {
    ...
    pub fn buttons_len(&self) -> u16 {
        self.buttons.len().try_into().unwrap()
    }
}
...
impl TryParse for HierarchyChangeDataAddMaster {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining
            .get(misalignment..)
            .ok_or(ParseError::InsufficientData)?;
        let result = HierarchyChangeDataAddMaster {
            send_core,
            enable,
            name,
        };
...
impl TryParse for HierarchyChangeDataRemoveMaster {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (return_keyboard, remaining) = DeviceId::try_parse(remaining)?;
        let return_mode = return_mode.into();
        let result = HierarchyChangeDataRemoveMaster {
            deviceid,
            return_mode,
            return_pointer,
            return_keyboard,
...
impl HierarchyChangeData {
    fn try_parse(value: &[u8], type_: u16) -> Result<(Self, &[u8]), ParseError> {
        ...
        if switch_expr == u16::from(HierarchyChangeType::ADD_MASTER) {
            let (add_master, new_remaining) =
                HierarchyChangeDataAddMaster::try_parse(outer_remaining)?;
            outer_remaining = new_remaining;
            assert!(
                parse_result.is_none(),
                "The XML should prevent more than one 'if' from matching"
        ...
        if switch_expr == u16::from(HierarchyChangeType::REMOVE_MASTER) {
            let (remove_master, new_remaining) =
                HierarchyChangeDataRemoveMaster::try_parse(outer_remaining)?;
            outer_remaining = new_remaining;
            assert!(
                parse_result.is_none(),
                "The XML should prevent more than one 'if' from matching"
        ...
        if switch_expr == u16::from(HierarchyChangeType::ATTACH_SLAVE) {
            let (attach_slave, new_remaining) =
                HierarchyChangeDataAttachSlave::try_parse(outer_remaining)?;
            outer_remaining = new_remaining;
            assert!(
                parse_result.is_none(),
                "The XML should prevent more than one 'if' from matching"
        ...
        if switch_expr == u16::from(HierarchyChangeType::DETACH_SLAVE) {
            let (detach_slave, new_remaining) =
                HierarchyChangeDataDetachSlave::try_parse(outer_remaining)?;
            outer_remaining = new_remaining;
            assert!(
                parse_result.is_none(),
                "The XML should prevent more than one 'if' from matching"
...
impl HierarchyChangeData {
    #[allow(dead_code)]
    fn serialize(&self, type_: u16) -> impl AsRef<[u8]> {
        let mut result = Vec::new();
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, type_: u16) {
        ...
        match self {
            ...
            HierarchyChangeData::InvalidValue(_) => {
                panic!("attempted to serialize invalid switch case")
            }
        }
    }
...
impl<'input> XIChangeHierarchyRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let length_so_far = 0;
        let num_changes =
            u8::try_from(self.changes.len()).expect("`changes` has too many elements");
        ...
        let referenced: &[u8] = changes_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(changes_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for XIGetClientPointerReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = XIGetClientPointerReply {
            sequence,
            length,
            set,
            deviceid,
...
crate::bitmask_binop!(XIEventMask, u32);
...
impl TryParse for EventMask {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (mask, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, mask_len.try_to_usize()?)?;
...
impl<'input> XISelectEventsRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = masks_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(masks_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for XIQueryVersionReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = XIQueryVersionReply {
            sequence,
            length,
            major_version,
            minor_version,
...
impl core::fmt::Debug for DeviceType {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::MASTER_POINTER.0.into(),
                "MASTER_POINTER",
                "MasterPointer",
            ),
            (
                Self::MASTER_KEYBOARD.0.into(),
                "MASTER_KEYBOARD",
                "MasterKeyboard",
            ),
            (
                Self::SLAVE_POINTER.0.into(),
                "SLAVE_POINTER",
                "SlavePointer",
            ),
            (
                Self::SLAVE_KEYBOARD.0.into(),
                "SLAVE_KEYBOARD",
                "SlaveKeyboard",
            ),
            (
                Self::FLOATING_SLAVE.0.into(),
                "FLOATING_SLAVE",
                "FloatingSlave",
            ),
        ];
...
crate::bitmask_binop!(ScrollFlags, u8);
...
impl TryParse for ButtonClass {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (num_buttons, remaining) = u16::try_parse(remaining)?;
        let (state, remaining) = crate::x11_utils::parse_list::<u32>(
            remaining,
            u32::from(num_buttons)
                .checked_add(31u32)
                .ok_or(ParseError::InvalidExpression)?
                .checked_div(32u32)
                .ok_or(ParseError::InvalidExpression)?
        ...
        let (labels, remaining) =
            crate::x11_utils::parse_list::<xproto::Atom>(remaining, num_buttons.try_to_usize()?)?;
        let type_ = type_.into();
        let result = ButtonClass {
            type_,
            len,
            sourceid,
            state,
...
impl TryParse for KeyClass {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (num_keys, remaining) = u16::try_parse(remaining)?;
        let (keys, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, num_keys.try_to_usize()?)?;
        let type_ = type_.into();
        let result = KeyClass {
            type_,
            len,
...
impl TryParse for DeviceClassDataValuator {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(3..).ok_or(ParseError::InsufficientData)?;
        let mode = mode.into();
        let result = DeviceClassDataValuator {
            number,
            label,
            min,
            max,
...
impl TryParse for DeviceClassDataScroll {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (increment, remaining) = Fp3232::try_parse(remaining)?;
        let scroll_type = scroll_type.into();
        let result = DeviceClassDataScroll {
            number,
            scroll_type,
            flags,
            increment,
...
impl Serialize for DeviceClassDataTouch {
    ...
    fn serialize(&self) -> [u8; 2] {
        let mode_bytes = u8::from(self.mode).serialize();
        let num_touches_bytes = self.num_touches.serialize();
        [mode_bytes[0], num_touches_bytes[0]]
    }
    fn serialize_into(&self, bytes: &mut Vec<u8>) {
        bytes.reserve(2);
        u8::from(self.mode).serialize_into(bytes);
...
impl DeviceClassData {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, type_: u16) {
        ...
        match self {
            ...
            DeviceClassData::InvalidValue(_) => {
                panic!("attempted to serialize invalid switch case")
            }
        }
    }
...
impl TryParse for XIDeviceInfo {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining
            ...
            .ok_or(ParseError::InsufficientData)?;
        let (classes, remaining) =
            crate::x11_utils::parse_list::<DeviceClass>(remaining, num_classes.try_to_usize()?)?;
        let type_ = type_.into();
        let result = XIDeviceInfo {
            deviceid,
            type_,
            attachment,
            enabled,
...
impl TryParse for XIQueryDeviceReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (infos, remaining) =
            crate::x11_utils::parse_list::<XIDeviceInfo>(remaining, num_infos.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = XIQueryDeviceReply {
            sequence,
            length,
            infos,
        };
...
impl XIQueryDeviceReply {
    ...
    pub fn num_infos(&self) -> u16 {
        self.infos.len().try_into().unwrap()
    }
}
...
impl<'input> XIGrabDeviceRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let length_so_far = length_so_far + request0.len();
        let mask_bytes = self.mask.serialize();
        let referenced: &[u8] = mask_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(mask_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl core::fmt::Debug for EventMode {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            ...
            (
                Self::REPLAY_DEVICE.0.into(),
                "REPLAY_DEVICE",
                "ReplayDevice",
            ),
            (
                Self::ASYNC_PAIRED_DEVICE.0.into(),
                "ASYNC_PAIRED_DEVICE",
                "AsyncPairedDevice",
            ),
...
impl core::fmt::Debug for ModifierMask {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [(Self::ANY.0, "ANY", "Any")];
        pretty_print_bitmask(fmt, self.0, &variants)
    }
}
crate::bitmask_binop!(ModifierMask, u32);
...
impl<'input> XIPassiveGrabDeviceRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let deviceid_bytes = self.deviceid.serialize();
        let num_modifiers =
            u16::try_from(self.modifiers.len()).expect("`modifiers` has too many elements");
        ...
        let length_so_far = length_so_far + referenced.len();
        let modifiers_bytes = self.modifiers.serialize();
        let referenced: &[u8] = modifiers_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        ...
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(mask_bytes.as_ref());
        request0.extend_from_slice(modifiers_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for XIPassiveGrabDeviceReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (modifiers, remaining) = crate::x11_utils::parse_list::<GrabModifierInfo>(
            remaining,
            num_modifiers.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = XIPassiveGrabDeviceReply {
            sequence,
            length,
            modifiers,
        };
...
impl XIPassiveGrabDeviceReply {
    ...
    pub fn num_modifiers(&self) -> u16 {
        self.modifiers.len().try_into().unwrap()
    }
}
...
impl TryParse for XIListPropertiesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (properties, remaining) = crate::x11_utils::parse_list::<xproto::Atom>(
            remaining,
            num_properties.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = XIListPropertiesReply {
            sequence,
            length,
            properties,
        };
...
impl XIChangePropertyAux {
    ...
    fn serialize_into(&self, bytes: &mut Vec<u8>, format: u8, num_items: u32) {
        ...
        match self {
            ...
            XIChangePropertyAux::InvalidValue(_) => {
                panic!("attempted to serialize invalid switch case")
            }
        }
    }
...
impl TryParse for XIGetSelectedEventsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(22..).ok_or(ParseError::InsufficientData)?;
        let (masks, remaining) =
            crate::x11_utils::parse_list::<EventMask>(remaining, num_masks.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = XIGetSelectedEventsReply {
            sequence,
            length,
            masks,
        };
...
impl XIGetSelectedEventsReply {
    ...
    pub fn num_masks(&self) -> u16 {
        self.masks.len().try_into().unwrap()
    }
}
...
impl TryParse for BarrierReleasePointerInfo {
    fn try_parse(remaining: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = BarrierReleasePointerInfo {
            deviceid,
            barrier,
            eventid,
        };
...
impl<'input> XIBarrierReleasePointerRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        let length_so_far = 0;
        let num_barriers =
            u32::try_from(self.barriers.len()).expect("`barriers` has too many elements");
        ...
        let referenced: &[u8] = barriers_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let padding0 = &[0; 3][..(4 - (length_so_far % 4)) % 4];
        let length_so_far = length_so_far + padding0.len();
        debug_assert_eq!(0, length_so_far % 4);
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(barriers_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
}
...
impl TryParse for DeviceValuatorEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = DeviceValuatorEvent {
            response_type,
            device_id,
            sequence,
            device_state,
...
impl core::fmt::Debug for MoreEventsMask {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [(Self::MORE_EVENTS.0.into(), "MORE_EVENTS", "MoreEvents")];
        pretty_print_bitmask(fmt, self.0.into(), &variants)
    }
}
crate::bitmask_binop!(MoreEventsMask, u8);
...
impl TryParse for DeviceKeyPressEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (same_screen, remaining) = bool::try_parse(remaining)?;
        let (device_id, remaining) = u8::try_parse(remaining)?;
        let result = DeviceKeyPressEvent {
            response_type,
            detail,
            sequence,
            time,
...
impl core::fmt::Debug for ClassesReportedMask {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            ...
            (
                Self::REPORTING_VALUATORS.0.into(),
                "REPORTING_VALUATORS",
                "ReportingValuators",
            ),
            (
                Self::REPORTING_BUTTONS.0.into(),
                "REPORTING_BUTTONS",
                "ReportingButtons",
            ),
            (
                Self::REPORTING_KEYS.0.into(),
                "REPORTING_KEYS",
                "ReportingKeys",
            ),
        ];
...
crate::bitmask_binop!(ClassesReportedMask, u8);
...
impl TryParse for DeviceStateNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let valuators = [valuators_0, valuators_1, valuators_2];
        let result = DeviceStateNotifyEvent {
            response_type,
            device_id,
            sequence,
            time,
...
impl TryParse for DeviceMappingNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = DeviceMappingNotifyEvent {
            response_type,
            device_id,
            sequence,
            request,
...
impl TryParse for ChangeDeviceNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(23..).ok_or(ParseError::InsufficientData)?;
        let request = request.into();
        let result = ChangeDeviceNotifyEvent {
            response_type,
            device_id,
            sequence,
            time,
...
impl TryParse for DeviceKeyStateNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = DeviceKeyStateNotifyEvent {
            response_type,
            device_id,
            sequence,
            keys,
...
impl TryParse for DeviceButtonStateNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = DeviceButtonStateNotifyEvent {
            response_type,
            device_id,
            sequence,
            buttons,
...
impl core::fmt::Debug for DeviceChange {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            ...
            (
                Self::UNRECOVERABLE.0.into(),
                "UNRECOVERABLE",
                "Unrecoverable",
            ),
            (
                Self::CONTROL_CHANGED.0.into(),
                "CONTROL_CHANGED",
                "ControlChanged",
            ),
        ];
...
impl TryParse for DevicePresenceNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = DevicePresenceNotifyEvent {
            response_type,
            sequence,
            time,
            devchange,
...
impl TryParse for DevicePropertyNotifyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = DevicePropertyNotifyEvent {
            response_type,
            state,
            sequence,
            time,
...
impl core::fmt::Debug for KeyEventFlags {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [(Self::KEY_REPEAT.0, "KEY_REPEAT", "KeyRepeat")];
        pretty_print_bitmask(fmt, self.0, &variants)
    }
}
crate::bitmask_binop!(KeyEventFlags, u32);
...
impl TryParse for KeyPressEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (button_mask, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, buttons_len.try_to_usize()?)?;
        let (valuator_mask, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, valuators_len.try_to_usize()?)?;
        let (axisvalues, remaining) = crate::x11_utils::parse_list::<Fp3232>(
            remaining,
            valuator_mask
                .iter()
                .try_fold(0u32, |acc, x| {
                    acc.checked_add((*x).count_ones())
                        .ok_or(ParseError::InvalidExpression)
                })?
                .try_to_usize()?,
        )?;
        let result = KeyPressEvent {
            response_type,
            extension,
            sequence,
            length,
...
impl KeyPressEvent {
    ...
    #[must_use]
    pub fn buttons_len(&self) -> u16 {
        self.button_mask.len().try_into().unwrap()
    }
    ...
    pub fn valuators_len(&self) -> u16 {
        self.valuator_mask.len().try_into().unwrap()
    }
}
...
crate::bitmask_binop!(PointerEventFlags, u32);
...
impl core::fmt::Debug for NotifyMode {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            ...
            (
                Self::WHILE_GRABBED.0.into(),
                "WHILE_GRABBED",
                "WhileGrabbed",
            ),
            (Self::PASSIVE_GRAB.0.into(), "PASSIVE_GRAB", "PassiveGrab"),
            (
                Self::PASSIVE_UNGRAB.0.into(),
                "PASSIVE_UNGRAB",
                "PassiveUngrab",
            ),
        ];
...
impl core::fmt::Debug for HierarchyMask {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            ...
            (
                Self::MASTER_REMOVED.0.into(),
                "MASTER_REMOVED",
                "MasterRemoved",
            ),
            (Self::SLAVE_ADDED.0.into(), "SLAVE_ADDED", "SlaveAdded"),
            (
                Self::SLAVE_REMOVED.0.into(),
                "SLAVE_REMOVED",
                "SlaveRemoved",
            ),
            (
                Self::SLAVE_ATTACHED.0.into(),
                "SLAVE_ATTACHED",
                "SlaveAttached",
            ),
            (
                Self::SLAVE_DETACHED.0.into(),
                "SLAVE_DETACHED",
                "SlaveDetached",
            ),
            (
                Self::DEVICE_ENABLED.0.into(),
                "DEVICE_ENABLED",
                "DeviceEnabled",
            ),
            (
                Self::DEVICE_DISABLED.0.into(),
                "DEVICE_DISABLED",
                "DeviceDisabled",
            ),
        ];
...
crate::bitmask_binop!(HierarchyMask, u8);
...
impl TryParse for HierarchyEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(10..).ok_or(ParseError::InsufficientData)?;
        let (infos, remaining) =
            crate::x11_utils::parse_list::<HierarchyInfo>(remaining, num_infos.try_to_usize()?)?;
        let result = HierarchyEvent {
            response_type,
            extension,
            sequence,
            length,
...
impl TryParse for RawKeyPressEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (axisvalues_raw, remaining) = crate::x11_utils::parse_list::<Fp3232>(
            remaining,
            valuator_mask
                .iter()
                .try_fold(0u32, |acc, x| {
...
impl TryParse for RawButtonPressEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = RawButtonPressEvent {
            response_type,
            extension,
            sequence,
            length,
...
crate::bitmask_binop!(TouchEventFlags, u32);
...
impl core::fmt::Debug for TouchOwnershipFlags {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [(Self::NONE.0, "NONE", "None")];
...
impl TryParse for TouchOwnershipEvent {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = TouchOwnershipEvent {
            response_type,
            extension,
            sequence,
            length,
...
impl core::fmt::Debug for BarrierFlags {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::POINTER_RELEASED.0.into(),
                "POINTER_RELEASED",
                "PointerReleased",
            ),
            (
                Self::DEVICE_IS_GRABBED.0.into(),
                "DEVICE_IS_GRABBED",
                "DeviceIsGrabbed",
            ),
        ];
...
crate::bitmask_binop!(BarrierFlags, u8);
...
impl<'input> SendExtensionEventRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = events_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        let classes_bytes = self.classes.serialize();
        let referenced: &[u8] = classes_bytes.as_ref();
        let length_so_far = length_so_far + referenced.len();
        ...
        let length = u16::try_from(length_so_far / 4).unwrap_or(0);
        request0[2..4].copy_from_slice(&length.to_ne_bytes());
        request0.extend_from_slice(events_bytes.as_ref());
        request0.extend_from_slice(classes_bytes.as_ref());
        request0.extend_from_slice(padding0.as_ref());
        request0
    }
