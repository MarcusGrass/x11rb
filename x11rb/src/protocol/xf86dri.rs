...
impl TryParse for QueryDirectRenderingCapableReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = QueryDirectRenderingCapableReply {
            sequence,
            length,
            is_capable,
        };
...
impl TryParse for OpenConnectionReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (bus_id, remaining) =
            crate::x11_utils::parse_u8_list(remaining, bus_id_len.try_to_usize()?)?;
        let bus_id = bus_id.to_vec();
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = OpenConnectionReply {
            sequence,
            length,
            sarea_handle_low,
            sarea_handle_high,
...
impl OpenConnectionReply {
    ...
    pub fn bus_id_len(&self) -> u32 {
        self.bus_id.len().try_into().unwrap()
    }
}
...
impl TryParse for GetClientDriverNameReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (client_driver_name, remaining) =
            crate::x11_utils::parse_u8_list(remaining, client_driver_name_len.try_to_usize()?)?;
        let client_driver_name = client_driver_name.to_vec();
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetClientDriverNameReply {
            sequence,
            length,
...
impl GetClientDriverNameReply {
    ...
    pub fn client_driver_name_len(&self) -> u32 {
        self.client_driver_name.len().try_into().unwrap()
    }
}
...
impl TryParse for CreateDrawableReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = CreateDrawableReply {
            sequence,
            length,
            hw_drawable_handle,
        };
...
impl TryParse for GetDrawableInfoReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (clip_rects, remaining) =
            crate::x11_utils::parse_list::<DrmClipRect>(remaining, num_clip_rects.try_to_usize()?)?;
        let (back_clip_rects, remaining) = crate::x11_utils::parse_list::<DrmClipRect>(
            remaining,
            num_back_clip_rects.try_to_usize()?,
        )?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetDrawableInfoReply {
            sequence,
            length,
...
impl GetDrawableInfoReply {
    ...
    pub fn num_clip_rects(&self) -> u32 {
        self.clip_rects.len().try_into().unwrap()
    }
    ...
    pub fn num_back_clip_rects(&self) -> u32 {
        self.back_clip_rects.len().try_into().unwrap()
    }
}
...
impl TryParse for GetDeviceInfoReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (device_private, remaining) =
            crate::x11_utils::parse_list::<u32>(remaining, device_private_size.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
...
impl GetDeviceInfoReply {
    ...
    pub fn device_private_size(&self) -> u32 {
        self.device_private.len().try_into().unwrap()
    }
}
...
impl TryParse for AuthConnectionReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = AuthConnectionReply {
            sequence,
            length,
            authenticated,
        };
