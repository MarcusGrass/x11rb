...
impl core::fmt::Debug for ModeFlag {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [
            (
                Self::POSITIVE_H_SYNC.0.into(),
                "POSITIVE_H_SYNC",
                "PositiveHSync",
            ),
            (
                Self::NEGATIVE_H_SYNC.0.into(),
                "NEGATIVE_H_SYNC",
                "NegativeHSync",
            ),
            (
                Self::POSITIVE_V_SYNC.0.into(),
                "POSITIVE_V_SYNC",
                "PositiveVSync",
            ),
            (
                Self::NEGATIVE_V_SYNC.0.into(),
                "NEGATIVE_V_SYNC",
                "NegativeVSync",
            ),
            (Self::INTERLACE.0.into(), "INTERLACE", "Interlace"),
            (
                Self::COMPOSITE_SYNC.0.into(),
                "COMPOSITE_SYNC",
                "CompositeSync",
            ),
            (
                Self::POSITIVE_C_SYNC.0.into(),
                "POSITIVE_C_SYNC",
                "PositiveCSync",
            ),
            (
                Self::NEGATIVE_C_SYNC.0.into(),
                "NEGATIVE_C_SYNC",
                "NegativeCSync",
            ),
...
impl core::fmt::Debug for ClockFlag {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let variants = [(Self::PROGRAMABLE.0.into(), "PROGRAMABLE", "Programable")];
        pretty_print_bitmask(fmt, self.0.into(), &variants)
    }
}
crate::bitmask_binop!(ClockFlag, u8);
...
crate::bitmask_binop!(Permission, u8);
...
impl TryParse for GetModeLineReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (private, remaining) =
            crate::x11_utils::parse_u8_list(remaining, privsize.try_to_usize()?)?;
        let private = private.to_vec();
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetModeLineReply {
            sequence,
            length,
            dotclock,
            hdisplay,
...
impl GetModeLineReply {
    ...
    pub fn privsize(&self) -> u32 {
        self.private.len().try_into().unwrap()
    }
}
...
impl<'input> ModModeLineRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        let referenced: &[u8] = self.private.as_ref();
        ...
        request0.extend_from_slice(self.private.as_ref());
...
impl TryParse for GetMonitorReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (hsync, remaining) =
            crate::x11_utils::parse_list::<Syncrange>(remaining, num_hsync.try_to_usize()?)?;
        let (vsync, remaining) =
            crate::x11_utils::parse_list::<Syncrange>(remaining, num_vsync.try_to_usize()?)?;
        let (vendor, remaining) =
            crate::x11_utils::parse_u8_list(remaining, vendor_length.try_to_usize()?)?;
        let vendor = vendor.to_vec();
        let (alignment_pad, remaining) = crate::x11_utils::parse_u8_list(
            remaining,
            (u32::from(vendor_length)
                .checked_add(3u32)
                .ok_or(ParseError::InvalidExpression)?
                & (!3u32))
                .checked_sub(u32::from(vendor_length))
                .ok_or(ParseError::InvalidExpression)?
                .try_to_usize()?,
        )?;
        let alignment_pad = alignment_pad.to_vec();
        let (model, remaining) =
            crate::x11_utils::parse_u8_list(remaining, model_length.try_to_usize()?)?;
        let model = model.to_vec();
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetMonitorReply {
            sequence,
            length,
            hsync,
            vsync,
...
impl GetMonitorReply {
    ...
    pub fn model_length(&self) -> u8 {
        self.model.len().try_into().unwrap()
    }
    ...
    pub fn num_hsync(&self) -> u8 {
        self.hsync.len().try_into().unwrap()
    }
    ...
    pub fn num_vsync(&self) -> u8 {
        self.vsync.len().try_into().unwrap()
    }
}
...
impl TryParse for GetAllModeLinesReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(20..).ok_or(ParseError::InsufficientData)?;
        let (modeinfo, remaining) =
            crate::x11_utils::parse_list::<ModeInfo>(remaining, modecount.try_to_usize()?)?;
        if response_type != 1 {
            return Err(ParseError::InvalidValue);
        }
        let result = GetAllModeLinesReply {
            sequence,
            length,
            modeinfo,
        };
...
impl GetAllModeLinesReply {
    ...
    pub fn modecount(&self) -> u32 {
        self.modeinfo.len().try_into().unwrap()
    }
}
...
impl TryParse for ValidateModeLineReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = ValidateModeLineReply {
            sequence,
            length,
            status,
        };
...
impl TryParse for GetDotClocksReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let remaining = remaining.get(12..).ok_or(ParseError::InsufficientData)?;
        let (clock, remaining) = crate::x11_utils::parse_list::<u32>(
            remaining,
            1u32.checked_sub(flags & 1u32)
                .ok_or(ParseError::InvalidExpression)?
                .checked_mul(clocks)
                .ok_or(ParseError::InvalidExpression)?
                .try_to_usize()?,
...
impl TryParse for GetGammaRampReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let (red, remaining) = crate::x11_utils::parse_list::<u16>(
            remaining,
            (u32::from(size)
                .checked_add(1u32)
                .ok_or(ParseError::InvalidExpression)?
        ...
        let (green, remaining) = crate::x11_utils::parse_list::<u16>(
            remaining,
            (u32::from(size)
                .checked_add(1u32)
                .ok_or(ParseError::InvalidExpression)?
        ...
        let (blue, remaining) = crate::x11_utils::parse_list::<u16>(
            remaining,
            (u32::from(size)
                .checked_add(1u32)
                .ok_or(ParseError::InvalidExpression)?
...
impl<'input> SetGammaRampRequest<'input> {
    ...
    pub fn serialize(self, major_opcode: u8) -> impl AsRef<[u8]> {
        ...
        debug_assert_eq!(
            self.red.len(),
            usize::try_from(u32::from(self.size).checked_add(1u32).unwrap() & (!1u32)).unwrap(),
            "`red` has an incorrect length"
        );
        let red_bytes = self.red.serialize();
...
impl TryParse for GetGammaRampSizeReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetGammaRampSizeReply {
            sequence,
            length,
            size,
        };
...
impl TryParse for GetPermissionsReply {
    fn try_parse(initial_value: &[u8]) -> Result<(Self, &[u8]), ParseError> {
        ...
        let result = GetPermissionsReply {
            sequence,
            length,
            permissions,
        };
