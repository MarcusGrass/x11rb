...
pub struct ExtensionInfoProvider {
    pub(crate) extensions: heapless::Vec<(String, ExtensionInformation), 32>,
}

impl ExtensionInfoProvider {
    pub(crate) fn get_by_name(&self, name: &str) -> Option<ExtensionInformation> {
        self.extensions
            .iter()
            .find_map(|(ext_name, ext)| (ext_name == name).then(|| *ext))
    }

    pub(crate) fn get_from_major_opcode(
        &self,
        major_opcode: u8,
    ...
    pub(crate) fn get_from_event_code(
        &self,
        event_code: u8,
    ) -> Option<(&str, ExtensionInformation)> {
        self.extensions
            ...
            .filter_map(|(name, ext_info)| {
                (ext_info.first_event <= event_code).then(|| (name.as_str(), *ext_info))
            })
            .max_by_key(|i| i.1.first_event)
    }

    pub(crate) fn get_from_error_code(
        &self,
        error_code: u8,
    ) -> Option<(&str, ExtensionInformation)> {
        self.extensions
            .iter()
            .filter_map(|(name, ext_info)| {
                (ext_info.first_error <= error_code).then(|| (name.as_str(), *ext_info))
            })
...
pub trait TryParseFd: Sized {
    ...
    /// If parsing is successful, an instance of the type and a slice for the remaining data should
    /// be returned. Otherwise, an error is returned.
    fn try_parse_fd(value: &[u8]) -> Result<(Self, &[u8]), ParseError>;
}
